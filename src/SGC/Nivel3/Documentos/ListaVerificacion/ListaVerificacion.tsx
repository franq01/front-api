...
import { FormDataEncabezadoListaVerificacion } from "./Types";
import { useCrud } from "../../../../libs/useCrud";
import ModalListaVerificacion, {
  createEncabezadoListaVerificacion,
} from "./ModalListaVerificacion";

const queryEncabezado = async (): Promise<
  FormDataEncabezadoListaVerificacion[]
> => (await axios.get("/api/listaVerificacion")).data;

const deleteEncabezado = async (id: string): Promise<void> =>
  await axios.delete(`/api/listaVerificacion/${id}`);

const onDownloadPDF = async (id: string) => {
  try {
    const response = await axios.get(
      `/api/listaVerificacion/reporte/pdf/${id}`,
      {
        responseType: "blob",
      }
    );
...
const ListaVerificacion = () => {
  const {
    ...
    error,
    deleteItem,
  } = useCrud<FormDataEncabezadoListaVerificacion>({
    ...
    createItem: createEncabezadoListaVerificacion, // Utiliza la función real para crear registros
    updateItem: () =>
      Promise.resolve({
        coDocumento: "",
        noRevision: "",
  ...
  const dataNueva = data.map((item) => ({
    ...item,
    id: String(item.idListaVerificacion),
  }));
