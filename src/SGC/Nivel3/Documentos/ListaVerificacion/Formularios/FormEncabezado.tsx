...
import {
  ...
  schemaEncabezadoListaVerificacion,
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoListaVerificacion | null>
  >;
};

export const FormEncabezado: React.FC<FormularioEncabezadoProps> = ({
  setEncabezadoData,
}) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoListaVerificacion>({
    resolver: zodResolver(schemaEncabezadoListaVerificacion),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoListaVerificacion> = (
    data
  ) => {
