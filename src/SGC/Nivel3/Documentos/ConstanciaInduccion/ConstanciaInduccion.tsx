...
import { FormDataEncabezadoConstanciaInduccion } from "./Type";
import { useCrud } from "../../../../libs/useCrud";
import ModalConstanciaInduccion, {
  createEncabezadoConsInduccion,
} from "./ModalConstanciaInduccion";

const queryEncabezado = async (): Promise<
  FormDataEncabezadoConstanciaInduccion[]
> => (await axios.get("/api/constanciaInduccion")).data;

const deleteEncabezado = async (id: string): Promise<void> =>
  await axios.delete(`/api/constanciaInduccion/${id}`);

const onDownloadPDF = async (id: string) => {
  try {
    const response = await axios.get(
      `/api/constanciaInduccion/reporte/pdf/${id}`,
      {
        responseType: "blob",
      }
    );
...
const ConstanciaInduccion = () => {
  const {
    items: data,
    isLoading,
    error,
    deleteItem,
  } = useCrud<FormDataEncabezadoConstanciaInduccion>({
    ...
    createItem: createEncabezadoConsInduccion, // Utiliza la función real para crear registros
    updateItem: () =>
      Promise.resolve({
        coDocumento: "",
        noRevision: "",
  ...
  const dataNueva = data.map((item) => ({
    ...item,
    id: String(item.idConstanciaInduccion),
  }));
...
export default ConstanciaInduccion;
