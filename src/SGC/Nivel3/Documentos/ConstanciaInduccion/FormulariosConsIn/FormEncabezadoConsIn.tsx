...
import {
  ...
  schemaEncabezadoConstanciaInduccion,
} from "../Type";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoConstanciaInduccion | null>
  >;
};

export const FormEncabezadoConsIn: React.FC<FormularioEncabezadoProps> = ({
  setEncabezadoData,
}) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoConstanciaInduccion>({
    resolver: zodResolver(schemaEncabezadoConstanciaInduccion),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoConstanciaInduccion> = (
    data
  ) => {
