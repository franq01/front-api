...
import {
  ...
  schemaEncabezadoMinutaReunion,
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoMinutaReunion | null>
  >;
};

export const FormEncabezadoMinutaReunion: React.FC<
...
> = ({ setEncabezadoData }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoMinutaReunion>({
    resolver: zodResolver(schemaEncabezadoMinutaReunion),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoMinutaReunion> = (data) => {
