...
import { FormDataEncabezadoMinutaReunion } from "./Types";
import { useCrud } from "../../../../libs/useCrud";
import ModalMinutaReunion, {
  createEncabezadoMinutaReunion,
} from "./ModalMinutaReunion";

const queryEncabezado = async (): Promise<FormDataEncabezadoMinutaReunion[]> =>
  (await axios.get("/api/minutaReunion")).data;

const deleteEncabezado = async (id: string): Promise<void> =>
  await axios.delete(`/api/minutaReunion/${id}`);

const onDownloadPDF = async (id: string) => {
  try {
    const response = await axios.get(`/api/minutaReunion/reporte/pdf/${id}`, {
      responseType: "blob",
    });
...
const MinutaReunion = () => {
  const {
    ...
    error,
    deleteItem,
  } = useCrud<FormDataEncabezadoMinutaReunion>({
    ...
    createItem: createEncabezadoMinutaReunion, // Utiliza la función real para crear registros
    updateItem: () =>
      Promise.resolve({
        coDocumento: "",
        noRevision: "",
  ...
  const dataNueva = data.map((item) => ({
    ...item,
    id: String(item.idMinutaReunion),
  }));
