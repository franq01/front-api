...
import {
  ...
  schemaEncabezadoReporteAuditoria,
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoReporteAuditoria | null>
  >;
};

export const FormEncabezadoReporteAuditoria: React.FC<
...
> = ({ setEncabezadoData }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoReporteAuditoria>({
    resolver: zodResolver(schemaEncabezadoReporteAuditoria),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoReporteAuditoria> = (
    data
  ) => {
