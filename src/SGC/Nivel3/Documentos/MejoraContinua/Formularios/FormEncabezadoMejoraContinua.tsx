...
import {
  FormDataEncabezadoMejoraContinua,
  schemaEncabezadoMejoraContinua,
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoMejoraContinua | null>
  >;
};

export const FormEncabezadoMejoraContinua: React.FC<
...
> = ({ setEncabezadoData }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoMejoraContinua>({
    resolver: zodResolver(schemaEncabezadoMejoraContinua),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoMejoraContinua> = (data) => {
