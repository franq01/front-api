...
import { FormDataEncabezadoDNC, schemaEncabezadoDNC } from "../Type";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoDNC | null>
  >;
};

export const FormEncabezadoDNC: React.FC<FormularioEncabezadoProps> = ({
  setEncabezadoData,
}) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoDNC>({
    resolver: zodResolver(schemaEncabezadoDNC),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoDNC> = (data) => {
