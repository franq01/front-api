...
import {
  ...
  schemaEncabezadoListaAsistencia,
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoListaAsistencia | null>
  >;
};

export const FormEncabezadoListaAsistencia: React.FC<
...
> = ({ setEncabezadoData }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoListaAsistencia>({
    resolver: zodResolver(schemaEncabezadoListaAsistencia),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoListaAsistencia> = (data) => {
