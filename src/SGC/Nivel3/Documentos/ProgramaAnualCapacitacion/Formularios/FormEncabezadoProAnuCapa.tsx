...
import {
  FormDataEncabezadoProAnuCapa,
  schemaEncabezadoProAnuCapa,
} from "../Type";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoProAnuCapa | null>
  >;
};

export const FormEncabezadoProAnuCapa: React.FC<FormularioEncabezadoProps> = ({
  setEncabezadoData,
}) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoProAnuCapa>({
    resolver: zodResolver(schemaEncabezadoProAnuCapa),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoProAnuCapa> = (data) => {
