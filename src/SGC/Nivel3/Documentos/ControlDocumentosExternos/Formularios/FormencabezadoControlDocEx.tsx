...
import { FormDataEncabezadoConDocEx, schemaEncabezadoConDocEx } from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoConDocEx | null>
  >;
};

export const FormencabezadoControlDocEx: React.FC<
...
> = ({ setEncabezadoData }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoConDocEx>({
    resolver: zodResolver(schemaEncabezadoConDocEx),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoConDocEx> = (data) => {
