...
import {
  ...
  schemaEncabezadoSeAcMeCoPre,
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoSeAcMeCoPre | null>
  >;
};

export const FormEncabezadoSeAcMeCoPre: React.FC<FormularioEncabezadoProps> = ({
  setEncabezadoData,
}) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoSeAcMeCoPre>({
    resolver: zodResolver(schemaEncabezadoSeAcMeCoPre),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoSeAcMeCoPre> = (data) => {
