...
schemaEncabezadoSolicitudPersonal
} from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: React.Dispatch<
    React.SetStateAction<FormDataEncabezadoSolicitudPersonal | null>
  >;
};

export const EncabezadoSolicitudPersonal: React.FC<
...
> = ({ setEncabezadoData }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoSolicitudPersonal>({
    resolver: zodResolver(schemaEncabezadoSolicitudPersonal),
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoSolicitudPersonal> = (data) => {
