...
import { FormDataProcesosLisMaDoc, schemaProcesos } from "../../Typos";
...
type Props = {
  onAddRecord: (data: FormDataProcesosLisMaDoc) => void;
};

const FormProcesos: React.FC<Props> = ({ onAddRecord }) => {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<FormDataProcesosLisMaDoc>({
    resolver: zodResolver(schemaProcesos),
  });

  const onSubmit: SubmitHandler<FormDataProcesosLisMaDoc> = (data) => {
