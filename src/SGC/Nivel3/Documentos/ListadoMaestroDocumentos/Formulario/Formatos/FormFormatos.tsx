...
import { FormDataFormatosLisMaDoc, schemaFormatos } from "../../Typos";
...
type Props = {
  onAddRecord: (data: FormDataFormatosLisMaDoc) => void;
};

const FormFormatos: React.FC<Props> = ({ onAddRecord }) => {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<FormDataFormatosLisMaDoc>({
    resolver: zodResolver(schemaFormatos),
  });

  const onSubmit: SubmitHandler<FormDataFormatosLisMaDoc> = (data) => {
