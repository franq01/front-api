...
import { FormDataEncabezadoBSC, schemaEncabezadoBSC } from "../Types";

type FormularioEncabezadoProps = {
  setEncabezadoData: (
    updatedPartes: FormDataEncabezadoBSC
  ) => Promise<void> | void; // Cambiartipo
  initialData?: FormDataEncabezadoBSC;
};

export const FormEncabezadoBSC: React.FC<FormularioEncabezadoProps> = ({
  setEncabezadoData,
  initialData,
}) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormDataEncabezadoBSC>({
    resolver: zodResolver(schemaEncabezadoBSC),
    defaultValues: initialData || {}, // Configura los valores iniciales si existen
  });

  const onSubmit: SubmitHandler<FormDataEncabezadoBSC> = (data) => {
